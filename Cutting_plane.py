...
def cut_rob_pmin(R,p,e_tol,utility,utility_eva,h_func,phi_func,h_eva,r,W0, par=0.5, par_u=1):
    ...
    c = cp.Variable(1)
    constraints = [cp.sum(a)==W0]
    #constraints = [cp.abs(a)<=W0, cp.sum(a)==1, W0*(1+(R @ a)) >= np.zeros(N)]
    iterations = 0
    ...
    while True:
        ...
        print('lb:',rbvalue,'ub:',c.value,'iter:',iterations)
        if rbvalue <= c.value+e_tol:
            return('sol:',w, 'lb:', prob.value,'ub:',rbvalue, 'iter:', iterations)
...
def cut_nom_pmin(R,p,e_tol,utility,utility_eva,h_eva,W0,par,par_u):
    N = len(p)
    I = len(R[0])
    a = cp.Variable(I, nonneg = True)
    c = cp.Variable(1)
...
def cut_rob_pmax(R,p,e_tol,utility,utility_eva,h_func,phi_func,h_eva,r, r_f, c, W0,par=2, par_u=1):
    ...
    while True:
        ...
        [rbvalue,h] = robustcheck(x,p,h_func,phi_func,r,par)
        print('constr_value:',rbvalue,c,'iter:',iterations)
        if rbvalue <= c+e_tol:
            return('sol:',w,'obj:', prob.value,'iter:', iterations)
